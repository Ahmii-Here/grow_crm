...
function NXBootCards() {
    ...
    if ($("#cardModal").length) {
         ...
         * UPDATING ASSIGNED USERS
         * ------------------------------------------------------------*/
        $(document).off('click', '#card-appointments-update-assigned').on('click', '#card-appointments-update-assigned', function () {
            //update the buttons parent popover
            $(this).attr('data-form-id', $(this).closest('.popover').attr('id'));
            //add loading class
            $("#task-assigned-container").html('');
    ...
    if ($("#cardModal").length) {
        ...
        function nxTimerAppointmentPolling() {
            if ($("#timerAppointmentPollingTrigger").length) {
                nxAjaxUxRequest($("#timerAppointmentPollingTrigger"));
            }
            setTimeout(nxTimerAppointmentPolling, 45000);
        };
        nxTimerAppointmentPolling();
    }
};
 ...
 * @description: validation for creating a lead
 * -------------------------------------------------------------------------------------*/
function NXAppointmentCreate() {

    //appoinment type selector
    $(".appoinment_type_selector").on('click', function () {
        $("#appoinment_type").val($(this).val());
    });
    ...
    $("#commonModalForm").validate({
        rules: {
            appoinment_title: "required",
            appoinment_projectid: "required",
            appoinment_priority: "required",
        },
        submitHandler: function (form) {
            //ajax form, so initiate ajax request here
            nxAjaxUxRequest($("#commonModalSubmitButton"));
 ...
 * @description: bootstrap the kanban board
 * -------------------------------------------------------------------------------------*/
function NXAppointmentsKanban() {

    if ($("#js-appointments-kanban-wrapper").length) {

        //position
        var appointment_position = $("#js-appointments-kanban-wrapper").attr('data-position');
        ...
        var cardsDraggable = dragula({
            ...
            .on('dragend', function (card) {
                ...
                //this card id
                var this_appointment_id = $(card).attr('data-appointment-id');

                //previous card's appointment  id
                var previous_list = $(card).prevAll()
                var previous_appointment_id = '';
                previous_list.each(function () {
                    if ($(this).hasClass('kanban-card')) {
                        previous_appointment_id = $(this).attr('data-appointment-id');
                        return false;
                    }
                });

                //next card's appointment id
                var next_appointment_id = $(card).next('.kanban-card').attr('data-appointment-id');
                if (typeof next_appointment_id == 'undefined') {
                    next_appointment_id = '';
                }
                ...
                var update_position = $.ajax({
                    ...
                    type: 'post',
                    url: appointment_position,
                    dataType: 'json',
                    data: 'appointment_id=' + this_appointment_id + '&previous_appointment_id=' + previous_appointment_id +
                        '&next_appointment_id=' + next_appointment_id + '&status=' + board_name,
                });
                */
...
function NXRecordMyTmeModal() {
    ...
    NX.recordAppointmentTimeToggle('disable');
...
function NXRecordMyTmeModalExtra() {
    ...
    NX.recordAppointmentTimeToggle('enable');
}
...
/**--------------------------------------------------------------------------------------
 * [CLONING A Appointment]
 * @description: validate
 * -------------------------------------------------------------------------------------*/
function NXCloneAppointment() {
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            appoinment_title: "required",
            appoinment_milestoneid: "required",
            project_id: "required",
...
/**--------------------------------------------------------------------------------------
 * [SETTINGS -appoinment STATUS]
 * @description: form aldation for lead status
 * -------------------------------------------------------------------------------------*/
function NXSettingsAppointmentStatus() {


    //change appoinment color - update form field
    $(document).on('change', '.appoinmentstatus_colors', function () {
        if (this.checked) {
            $("#appoinmentstatus_color").val($(this).val());
        }
    });
    ...
    $("#commonModalForm").validate({
        rules: {
            appoinmentstatus_title: "required"
        },
        submitHandler: function (form) {
            //set selector color
            $(".appoinmentstatus_colors").each(function () {
                if (this.checked) {
                    $("#appoinmentstatus_color").val($(this).val());
                }
            });
...
function NXSettingsAppointmentStatus() {
    ...
    //change task color - update form field
    $(document).on('change', '.appointmentstatus_colors', function () {
        if (this.checked) {
            $("#appointmentstatus_color").val($(this).val());
        }
    });
    ...
    $("#commonModalForm").validate({
        ...
        submitHandler: function (form) {
            //set selector color
            $(".appointmentstatus_colors").each(function () {
                if (this.checked) {
                    $("#appointmentstatus_color").val($(this).val());
                }
            });
...
/**--------------------------------------------------------------------------------------
 * [SETTINGS - APPOINMENT DRAG & DROP]
 * @description: drag and drop for leads
 * -------------------------------------------------------------------------------------*/
function NXSettingsAppointmentDragDrop() {
    ...
    stagesDraggable.on('dragend', function (stage) {
        ...
        //update the list
        nxAjaxUxRequest($("#appoinment-stages"));
...
/**--------------------------------------------------------------------------------------
 * [APPONTEMNT - RECURRING] 
 * @blade : tasks\components\modals\recurring-settings.blade.php
 * @description: validation for recurring an task
 * -------------------------------------------------------------------------------------*/
function NXAppointmentRecurring() {
    //validation
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            appointment_recurring_next: "required",
            appointment_recurring_duration: "required",
            appointment_recurring_period: "required",
            appointment_recurring_cycles: "required",
        },
        submitHandler: function (form) {
            //ajax form, so initiate ajax request here
            nxAjaxUxRequest($("#commonModalSubmitButton"));
 ...
 *   clear the client users list
 * -----------------------------------------------------------*/
function NXAppointmentProjectToggleClear($self) {
 ...
 * show client users in dropdown list
 * -----------------------------------------------------------*/
function NXAppointmentProjectToggle(e, $self) {
...
function NXCreateProductAppointment() {
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            product_appointment_title: "required"
        },
        submitHandler: function (form) {
            nxAjaxUxRequest($("#commonModalSubmitButton"));
        }
 ...
 * @description: form aldation for lead status
 * -------------------------------------------------------------------------------------*/
function NXSettingsAppointmentPriority() {


    //change task color - update form field
    $(document).on('change', '.appointmentpriority_colors', function () {
        if (this.checked) {
            $("#appointmentpriority_color").val($(this).val());
        }
    });
    ...
    $("#commonModalForm").validate({
        rules: {
            appointmentpriority_title: "required"
        },
        submitHandler: function (form) {
            //set selector color
            $(".appointmentpriority_colors").each(function () {
                if (this.checked) {
                    $("#appointmentpriority_color").val($(this).val());
                }
            });
