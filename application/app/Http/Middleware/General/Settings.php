...
class Settings {
    ...
    public function handle($request, Closure $next) {
        ...
        if (env('SETUP_STATUS') == 'COMPLETED') {
            ...
            config(['appointment_statuses' => $statuses]);
            $arr = [];
            foreach ($statuses as $status) {
                $arr[] = $status->appointmentstatus_id;
            }
            config(['appointment_statuses_array' => $arr]);
        }
        ...
        if (env('SETUP_STATUS') == 'COMPLETED') {
            ...
            config(['appointment_priorities' => $priorities]);
            $arr = [];
            foreach ($priorities as $priority) {
                $arr[] = $priority->appointmentpriority_id;
            }
            config(['appointment_priorities_array' => $arr]);
        }
