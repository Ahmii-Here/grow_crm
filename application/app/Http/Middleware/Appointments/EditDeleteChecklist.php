...
/** --------------------------------------------------------------------------------
 * This middleware class handles [edit checklist] precheck processes for appointments
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class EditDeleteChecklist {
    ...
    public function handle($request, Closure $next) {
        ...
        if (!$checklist = \App\Models\Checklist::Where('checklist_id', $checklist_id)->first()) {
            Log::error("checklist could not be found", ['process' => '[permissions][appointments][delete-checklist]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'checklist id' => $checklist_id ?? '']);
            
            //error - remove item with message
            $jsondata['dom_visibility'][] = array(
                'selector' => "#appointment_checklist_container_$checklist_id",
        ...
        //check permissions
        if ($checklist->checklistresource_type == 'appointment') {
        ...
        Log::error("permission denied", ['process' => '[permissions][appointments][delete-checklist]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'checklist id' => $checklist_id ?? '']);
        abort(403);
    }
}