...
class StoreResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (auth()->user()->pref_view_appointments_layout == 'list') {
            if ($count == 1) {
                $html = view('pages/appointments/components/table/table', compact('appointments'))->render();
        ...
        if (auth()->user()->pref_view_appointments_layout == 'kanban') {
            ...
            $jsondata['dom_html'][] = array(
                'selector' => '#kanban-board-wrapper-' . request('appointment_status'),
                'action' => 'prepend',
                'value' => $html);
        }
        ...
        //show appointment after adding
        if (request('redirect_lead')) {
            $jsondata['redirect_url'] = url("/leads");
        }else{
            if (request('ref') == 'quickadd' && request('show_after_adding') == 'on') {
                $jsondata['redirect_url'] = url("/appointments/v/" . $appointment->appointment_id . "/" . str_slug($appointment->appointment_title));
            }
        }
