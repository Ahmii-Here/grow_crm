...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index kanban] process for the appointments
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexKanbanResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'sort':
                $template = 'pages/appointments/components/kanban/wrapper';
                ...
                foreach ($boards as $board) {
                    ...
                    if (request('source') == $board_id) {
                        if ($appointments->currentPage() < $appointments->lastPage()) {
                            ...
                            $jsondata['dom_visibility'][] = array('selector' => '#appointments-loadmore-container-' . $board_id, 'action' => 'hide');
                        }
                    }
                }
            } else {
                foreach ($boards as $board) {
                    ...
                    if ($appointments->currentPage() < $appointments->lastPage()) {
                        ...
                        $jsondata['dom_attributes'][] = array(
                            ...
                            'value' => $url);
                        //load more - visible
                        //$jsondata['dom_visibility'][] = array('selector' => '#appointments-loadmore-container-' . $board_id, 'action' => 'show');
                    } else {
                        $jsondata['dom_visibility'][] = array('selector' => '#appointments-loadmore-container-' . $board_id, 'action' => 'hide');
                    }
                }
            ...
            if (request('action') == 'load') {
                ...
                if (request()->filled('source')) {
                    ...
                    $jsondata['dom_visibility'][] = [
                        'selector' => '#appointments-loadmore-container-' . $board_id,
                        'action' => 'hide-remove',
                    ];

                    //append a new button
                    $board['load_more_url'] = loadMoreButtonUrl($appointments->currentPage() + 1, request('source'));
                    $html = view('pages/appointments/components/kanban/loadmore-button', compact('board'))->render();
                    $jsondata['dom_html'][] = [
                        'selector' => '#kanban-board-wrapper-' . $board_id,
                        'action' => 'append',
                        'value' => $html,
            ...
            //filter show archvied appointments
            ...
            //standard view
            return view('pages/appointments/wrapper', compact('page', 'boards', 'stats', 'tags', 'milestones', 'statuses', 'fields', 'priorities'))->render();
        }
    }
}
