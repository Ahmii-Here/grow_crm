...
class Leads extends Controller {
    ...
    public function store(LeadStoreUpdate $request, LeadAssignedRepository $assignedrepo) {
        ...
        foreach(config('app.default-checklist') as $name){
            //get next position
            ...
            $customlead = Lead::findOrFail($lead_id);
            $customlead->checklists()->create(['position'=> $position, 'checklistresource_type'=> 'lead', 'checklistresource_id' => $lead_id, 'checklist_text'=>$name]);
        }
        ...
        foreach(config('app.leads-default-fields') as $name){
            $customlead = Lead::findOrFail($lead_id);
            $customlead->lead_fields()->create(['label'=> $name, 'value' => '']);
        }
    ...
    public function updateTitle($id) {
        ...
        if (hasHTML(request('lead_title'))) {
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => safestr($lead->lead_firstname . ' ' . $lead->lead_lastnamee),
            ];
            return response()->json($jsondata);
        }
        ...
        if (!request()->filled('lead_title')) {
            ...
            $jsondata['dom_html'][] = [
                'selector' => '#card-title-editable',
                'action' => 'replace',
                'value' => safestr($lead->lead_firstname . ' ' . $lead->lead_lastname),
            ];

            return response()->json($jsondata);

        } else {
            $explode = explode(' ', request('lead_title'));
            $lead->lead_firstname = $explode[0] ?? '';
            $lead->lead_lastname = str_replace($explode[0] ?? '', '', request('lead_title'));
            $lead->save();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => str_limit(safestr($lead->lead_firstname . ' ' . $lead->lead_lastname), 25),
            ];
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => str_limit(safestr($lead->lead_firstname . ' ' . $lead->lead_lastname), 45),
            ];
    ...
    public function showOrganisation($id) {
        ...
        if ($fields = \App\Models\LeadsExtraField::Where('field_lead_id', $id)
            ->exists()) {
            $has_field = true;
        } else {
            $fields = [];
        ...
        $fields = \App\Models\LeadsExtraField::Where('field_lead_id', $id)
                    ->get();
     ...
     * @return \Illuminate\Http\Response
     */
    public function showCustomFields(ChecklistRepository $checklistrepo, $id) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function showMyFields($id) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function editMyFields($id, $fieldId) {
        //get leads
        $field = \App\Models\LeadsExtraField::Where('field_lead_id', $id)
            ->Where('id', $fieldId)
            ->first();
        $fields = \App\Models\LeadsExtraField::Where('field_lead_id', $id)
                    ->get();
     ...
     * @return \Illuminate\Http\Response
     */
    public function deleteMyFields($id, $fieldId) {

        //delete all notes by this user
        \App\Models\LeadsExtraField::where('field_lead_id', $id)
            ->where('lead_id', $fieldId)->delete();
        $fields = \App\Models\LeadsExtraField::Where('field_lead_id', $id)
            ->get();
        //refeshed data
        $leads = $this->leadrepo->search($id);
        ...
        $payload = [
            ...
            'fields' => $fields,
            'lead' => $lead,
            'has_field' =>  \App\Models\LeadsExtraField::Where('field_lead_id', $id)->exists(),
        ];
     ...
     * @return \Illuminate\Http\Response
     */
    public function createMyFields($id) {

        //delete all notes by this user
        // \App\Models\LeadsExtraField::Where('noteresource_type', 'lead')
        //     ->where('field_lead_id', $id)
     ...
     * @return \Illuminate\Http\Response
     */
    public function updateMyFields($id, $fieldId = null) {

        //validation
        if (!request()->filled('lead_myfield_label') || !request()->filled('lead_myfield_value')) {
            abort(409, __('lang.fill_in_all_required_fields'));
        }
        $field = new \App\Models\LeadsExtraField();
        if ($fieldId) {
            $field = \App\Models\LeadsExtraField::findOrFail($fieldId);
        }

        //create note
        $field->field_lead_id = $id;
        $field->label = request('lead_myfield_label');
        $field->value = request('lead_myfield_value');
        $field->save();
    ...
    public function showMyNotes($id) {
        ...
        //get leads
        if ($notes = \App\Models\Note::Where('noteresource_type', 'lead')
            ->Where('noteresource_id', $id)
            ->Where('note_creatorid', auth()->id())->exists()) {
            $has_note = true;
        } else {
            $notes = [];
            $has_note = false;
        }
        $notes = \App\Models\Note::Where('noteresource_type', 'lead')
                    ->Where('noteresource_id', $id)
                    ->Where('note_creatorid', auth()->id())->get();
    ...
    public function showMyAppointments($id) {
        ...
        //get leads
        if ($appointments = \App\Models\Appointment::Where('lead_id', $id)
            ->Where('appointment_creatorid', auth()->id())->exists()) {
            $has_appointment = true;
        } else {
            $appointments = [];
            $has_appointment = false;
        }
        $appointments = \App\Models\Appointment::Where('lead_id', $id)->Where('appointment_creatorid', auth()->id())->get();
    ...
    public function showMyTasks($id) {
        ...
        //get leads
        if ($tasks = \App\Models\Task::Where('lead_id', $id)
            ->Where('task_creatorid', auth()->id())->exists()) {
            $has_task = true;
        } else {
            $tasks = [];
            $has_task = false;
        }
        $tasks = \App\Models\Task::Where('lead_id', $id)->Where('task_creatorid', auth()->id())->get();
     ...
     * @return \Illuminate\Http\Response
     */
    public function editMyNotes($id, $noteId) {
        //get leads
     ...
     * @return \Illuminate\Http\Response
     */
    public function deleteMyNotes($id, $noteId) {
        ...
        $payload = [
            ...
            'notes' => $notes,
            'lead' => $lead,
            'has_note' =>  \App\Models\Note::Where('noteresource_type', 'lead')
            ->Where('noteresource_id', $id)
            ->Where('note_creatorid', auth()->id())->exists(),
        ];
     ...
     * @return \Illuminate\Http\Response
     */
    public function updateMyNotes($id, $noteId = null) {
    ...
    public function cloneStore(LeadAssignedRepository $assignedrepo, $id) {
        ...
        $data = [
            ...
            'copy_fields' => (request('copy_fields') == 'on') ? true : false,
        ];
