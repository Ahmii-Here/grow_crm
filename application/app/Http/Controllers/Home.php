...
class Home extends Controller {
    ...
    public function adminDashboard() {
        ...
        $payload['count_checklists'] = [];

        foreach (config('app.default-checklist') as $name) {
            $leadsCount = Lead::whereHas('checklists', function ($q) use ($name) {
                $q->where('checklist_text', $name)->where('checklist_status', 'completed');
            })->count();
            $payload['count_checklists'][$name] = [
                'count' => $leadsCount ?? 0
            ];
        }
        ...
        foreach (config('app.default-checklist-dashboard') as $name) {
            $leadsCount = Lead::whereHas('checklists', function ($q) use ($name) {
                $q->where('checklist_text', $name)->where('checklist_status', 'completed');
            })->count();
            $payload['count_leads'][$name] = [
                'count' => $leadsCount ?? 0
            ];
        }
        //[leads] - alltime
