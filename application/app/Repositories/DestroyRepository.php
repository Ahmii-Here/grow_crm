...
class DestroyRepository {
    ...
    public function destroyAppointment($appointment_id) {

        //validate appointment
        if (!is_numeric($appointment_id)) {
            Log::error("validation error - invalid params", ['process' => '[destroy][appointment]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }

        //get appointment and validate
        if (!$appointment = \App\Models\Appointment::Where('appointment_id', $appointment_id)->first()) {
            Log::error("record could not be found", ['process' => '[destroy][appointment]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }

        //delete tags
        $appointment->tags()->delete();

        //delete checklists
        $appointment->checklists()->delete();

        //delete assigned records
        $appointment->assignedrecords()->delete();

        //delete attachemnts
        if ($attachments = $appointment->attachments()->get()) {
            foreach ($attachments as $attachment) {
                if ($attachment->attachment_directory != '') {
                    if (Storage::exists("files/$attachment->attachment_directory")) {
                        Storage::deleteDirectory("files/$attachment->attachment_directory");
        ...
        //delete timers
        $appointment->timers()->delete();

        //delete comments
        if ($comments = $appointment->comments()->get()) {
            foreach ($comments as $comment) {
                $this->destroyComment($comment->comment_id);
            }
        }
        ...
        //delete events and events tracking
        if ($events = \App\Models\Event::Where('event_parent_type', 'appointment')->Where('event_parent_id', $appointment_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
        ...
        //delete queued emails
        \App\Models\EmailQueue::Where('emailqueue_resourcetype', 'appointment')->Where('emailqueue_resourceid', $appointment_id)->delete();
