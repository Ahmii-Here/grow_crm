...
class LeadRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['apply_filters']) {
            ...
            //filter: lead_converted
            if ($data['lead_converted'] ?? false) {
                $leads->where('lead_converted', $data['lead_converted']);
            }

            //checklist status
            if (request('checklist')) {
                $leads->whereHas('checklists', function ($q) {
                    $q->where('checklist_text', request('checklist'))->where('checklist_status', 'completed');
                });
            }
    ...
    public function cloneLead($lead = '', $data = []) {
        ...
        //copy Copy Fields
        if ($data['copy_fields']) {
            if ($fields = \App\Models\LeadsExtraField::Where('field_lead_id', $lead->lead_id)->get()) {
                foreach ($fields as $field) {
                    $new = $field->replicate();
                    $new->field_lead_id = $new_lead->lead_id;
                    $new->save();
                }
            }
        }
