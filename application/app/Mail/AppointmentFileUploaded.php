...
/** --------------------------------------------------------------------------------
 * This classes renders the [appointment file uplaoded] email and stores it in the queue
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class AppointmentFileUploaded extends Mailable {
    use Queueable;
    ...
    public function build() {
        ...
        //email template
        if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Appointment File Uploaded')->first()) {
            return false;
        }
        ...
        $payload += [
            ...
            'file_url' => url('appointments/download-attachment/' . $this->data['attachment_uniqiueid']),
