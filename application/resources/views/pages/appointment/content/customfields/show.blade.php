...
@foreach($fields as $field) <div class="form-data-row">
        ...
        <span class="x-data-content {{ $field->customfields_datatype }}">{!!
            customFieldValueDisplay($field->customfields_name, $appoinment, $field->customfields_datatype) !!}</span>
...
<!--edit button-->
@if(config('visibility.appoinment_editing_buttons'))
<div class="form-data-row-buttons">
    <button type="button" class="btn waves-effect waves-light btn-xs btn-info ajax-request"
        data-url="{{ url('appoinments/content/'.$appoinment->appoinment_id.'/edit-customfields') }}"
        data-loading-class="loading-before-centre"
        data-loading-target="card-appoinments-left-panel">@lang('lang.edit')</button>
</div>
@endif
...
<div class="x-no-result">
    ...
    <div class="p-t-10">
        <a href="{{ url('app/settings/customfields/appoinments') }}" class="btn btn-info btn-sm">@lang('lang.create_custom_fields')</a>
    </div>
    @endif
</div>
@endif
