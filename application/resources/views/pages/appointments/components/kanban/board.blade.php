<div class="board">
    <div class="board-body {{ runtimeKanbanBoardColors($board['color']) }}">
        <div class="board-heading clearfix">
            ...
            <div class="pull-right x-action-icons">
                ...
                <span class="edit-add-modal-button js-ajax-ux-request reset-target-modal-form cursor-pointer"
                    ...
                    data-url="{{ urlResource('/appointments/create?status='.$board['id']) }}"
                    data-loading-target="commonModalBody" data-modal-title="{{ cleanLang(__('lang.add_appointment')) }}"
                    data-action-url="{{ urlResource('/appointments?type=kanban') }}" data-action-method="POST"
        ...
        <div class="content kanban-content" id="kanban-board-wrapper-{{ $board['id'] }}" data-board-name="{{ $board['id'] }}">
            ...
            <!--dynamic content-->
            @if(@count($board['appointments'] ?? []) > 0)
            @include('pages.appointments.components.kanban.card')
            @endif

            <!-- dynamic load more button-->
            @include('pages.appointments.components.kanban.loadmore-button')
        </div>
    </div>
</div>