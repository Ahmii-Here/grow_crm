...
<div class="kanban-card kanban-board-element show-modal-button reset-card-modal-form js-ajax-ux-request"
    ...
    <div class="x-meta">
        ...
        <span title="{{ ($task->lead->lead_firstname ?? '') . ' ' . ($task->lead->lead_lastname ?? '')  }}"><strong>{{ cleanLang(__('lang.lead')) }}:</strong>
            {{ str_limit(($task->lead->lead_firstname ?? '') . ' ' . ($task->lead->lead_lastname ?? '') , 68) }}</span>
        <!--project-->
        @if(1==2 && config('system.settings_tasks_kanban_project_title') == 'show')
        <span title="{{ $task->project_title ?? '---' }}"><strong>{{ cleanLang(__('lang.project')) }}:</strong>
            {{ str_limit($task->project_title ??'---', 68) }}</span>
        @endif
        <!--client-->
        @if(1==2 && config('system.settings_tasks_kanban_client_name') == 'show')
