...
<!--each row-->
<tr id="priority_{{ $priority->appointmentpriority_id }}">
    <td class="priority_col_date">
        <span class="mdi mdi-drag-vertical cursor-pointer"></span>
        <!--sorting data-->
        <input type="hidden" name="sort-priorities[{{ $priority->appointmentpriority_id }}]" value="{{ $priority->appointmentpriority_id }}">
        {{ runtimeLang($priority->appointmentpriority_title) }}
        <!--system initial stage-->
        @if($priority->appointmentpriority_system_default == 'yes' && $priority->appointmentpriority_id == 1)
    ...
    <td class="priority_col_count">{{ $priority->count_appointments }}</td>
    <td class="priority_col_color"><span class="bg-{{ $priority->appointmentpriority_color }}" id="fx-settimgs-appointments-priority">&nbsp;</span>
    </td>
    <td class="priority_col_created_by">
        <img src="{{ getUsersAvatar($priority->avatar_directory, $priority->avatar_filename, $priority->appointmentpriority_creatorid) }}" alt="user"
            class="img-circle avatar-xsmall">
            {{ checkUsersName($priority->first_name, $priority->appointmentpriority_creatorid)  }}
        </td>
    <td class="priority_col_action actions_column">
        ...
        <span class="list-table-action dropdown font-size-inherit" >
            <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                ...
                data-url="{{ url('/settings/appointments/priorities/'.$priority->appointmentpriority_id.'/edit') }}"
                data-loading-target="commonModalBody" data-modal-title="{{ cleanLang(__('lang.edit_appointment_priority')) }}"
                data-action-url="{{ url('/settings/appointments/priorities/'.$priority->appointmentpriority_id) }}" data-action-method="PUT"
            ...
            <button type="button" title="{{ cleanLang(__('lang.move')) }}"
                ...
                data-url="{{ url('/settings/appointments/move/priority/'.$priority->appointmentpriority_id) }}"
                data-loading-target="commonModalBody" data-modal-title="@lang('lang.move_appointments')"
                data-action-url="{{ url('/settings/appointments/move/priority/'.$priority->appointmentpriority_id) }}" data-action-method="PUT"
                data-action-ajax-class="js-ajax-ux-request" data-action-ajax-loading-target="commonModalBody">
                <i class="sl-icon-share-alt"></i>
            </button>
            @if($priority->appointmentpriority_system_default == 'no')
            <button type="button" title="{{ cleanLang(__('lang.delete')) }}" class="data-toggle-action-tooltip btn btn-outline-danger btn-circle btn-sm confirm-action-danger"
                data-confirm-title="{{ cleanLang(__('lang.delete_appointment_priority')) }}" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
                data-ajax-type="DELETE" data-url="{{ url('/') }}/settings/appointments/priorities/{{ $priority->appointmentpriority_id }}">
