...
    });
    ...
    $(document).on('change', '.lead-source-select', function(event) {
    	let value = $('.lead-source-select option:selected').val();

    	if (value == 'Other') {
    		$('.other-field').removeClass('d-none');
    		$('.other-field').find('input').attr('name', 'lead_source');
    		$(this).attr('name', false);
    	}else{
    		$(this).attr('name', 'lead_source');
    		$('.other-field').addClass('d-none');
    		$('.other-field').find('input').attr('name', false);
    	}
